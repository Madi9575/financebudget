...
const STORAGE_KEY = 'ego_data';
const PIN_KEY = 'ego_pin';
const SESSION_KEY = 'ego_session';
...
function AppInner() {
  ...
  const handleIdleLock = useCallback(() => {
    ...
    if (Date.now() - lastUnlockAtRef.current < 3000) {
      return;
    }
  ...
  useIdleLock({
    enabled: unlocked && !!localStorage.getItem(PIN_KEY),
    timeoutMs: IDLE_TIMEOUT_MS,
    onTimeout: handleIdleLock,
  });
  ...
  if (!unlocked) {
    return <LockScreen onUnlock={handleUnlock} pinOnly={loggedOut} />;
  }
      ...
      case 'budget':
        return (
          <BudgetView
            ...
            onReorderCategories={store.reorderBudgetCategories}
          />
        );
      case 'savings':
        return (
          <SavingsView
            ...
            onEditGoal={store.editSavingsGoal}
            onDeleteGoal={store.deleteSavingsGoal}
            onReorderGoals={store.reorderSavingsGoals}
          />
        );
      case 'transactions':
        return (
