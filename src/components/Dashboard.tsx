...
const BALANCE_HIDDEN_KEY = 'infinance_balance_hidden';
...
function formatLastUpdate(dateStr: string, locale: string): string {
  ...
  const dateLabel = d.toLocaleDateString(locale, { day: '2-digit', month: 'short' });

  if (diffMin < 1) return `${dateLabel} · ${time}`;
  if (diffMin < 60) return `${dateLabel} · ${diffMin}min · ${time}`;
  if (diffMin < 1440) return `${dateLabel} · ${Math.floor(diffMin / 60)}h · ${time}`;
