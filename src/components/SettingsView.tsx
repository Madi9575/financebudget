...
export default function SettingsView({ onReset, onLogout }: SettingsViewProps) {
  ...
  const [profile, setProfile] = useState(() => {
    ...
    return { name: 'Mon Espace', email: 'utilisateur@email.com', photo: '' };
  });
  ...
  const handleSetPin = () => {
    if (pinCode.length === 4) {
      localStorage.setItem('ego_pin', pinCode);
  ...
  const handlePhotoSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      showToast('Image trop volumineuse (max 2 Mo)');
      return;
    }
  ...
  return (
    <div className="pb-24">
      <div className="px-5 pt-8 pb-6">
        ...
        >
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-zinc-800 flex items-center justify-center text-xl overflow-hidden flex-shrink-0">
              {profile.photo ? (
      ...
      <div className="px-5">
        ...
        <p className="text-center text-zinc-300 text-[10px] uppercase tracking-widest">EGO v1.0</p>
      </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-4 mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-2xl bg-zinc-100 flex items-center justify-center text-2xl overflow-hidden flex-shrink-0 border border-zinc-200">
                    {editPhoto ? (
                  ...
                  <div className="flex-1">
                    <label className="block">
                      <span className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-zinc-100 text-zinc-700 text-xs font-medium cursor-pointer hover:bg-zinc-200 transition-colors">
                        {t('addPhoto')}
                      </span>
                      <input type="file" accept="image/*" onChange={handlePhotoSelect} className="hidden" />
                    </label>
                    {editPhoto && (
                      <button onClick={() => setEditPhoto('')} className="text-rose-500 text-xs mt-2">
                        {t('removePhoto')}
                      </button>
                    )}
                  </div>
