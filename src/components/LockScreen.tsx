...
export default function LockScreen({ onUnlock, pinOnly = false }: LockScreenProps) {
  const { t } = useLanguage();
  ...
  const handleBiometrics = useCallback(async () => {
    try {
      ...
      // Biometric failed or cancelled — do NOT unlock, user must use PIN
    }
  }, [onUnlock]);

  const showBiometrics = !pinOnly;
  const digits = ['1','2','3','4','5','6','7','8','9', showBiometrics ? 'bio' : 'empty', '0', 'del'];
      ...
      <div className="w-full max-w-md flex flex-col items-center justify-center px-10 py-16">
        ...
        >
          <div className="w-16 h-16 rounded-2xl bg-zinc-900 flex items-center justify-center mx-auto mb-5">
            <span className="text-white text-xl font-semibold tracking-tight">EGO</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-8 w-full max-w-[260px]">
          {digits.map(d => {
            ...
            if (d === 'empty') {
              return <div key={d} className="w-full aspect-square" />;
            }
            if (d === 'del') {
              return (
                <button
